...
class HanedaFlightScraper:
    ...
    async def get_flight_data(self):
        ...
        try:
            ...
            # フライトデータから有効な時刻のみを返す
            # フライトデータをターミナルで分けて返す
            # scheduledTime が存在し、空文字列でないことを確認してフィルタリング
            other_flights = [self.format_flight_time(flight) for flight in flights if flight.get('scheduledTime') and flight.get('scheduledTime').strip() and flight.get('gate') != 'T2']
            t2_flights = [self.format_flight_time(flight) for flight in flights if flight.get('scheduledTime') and flight.get('scheduledTime').strip() and flight.get('gate') == 'T2']
...
def generate_plot_internal(flight_data, title="Number of Arrivals by Time of Day (30-min Intervals)", filename_prefix="flight_graph"):
    """フライトデータからグラフを生成し、ファイルパスとBase64データを返す内部関数"""
    if not flight_data:
        print(f"グラフ生成スキップ: {title} - データなし")
        return None

    # スクレイパーインスタンスを作成 (時刻丸めのため)
    scraper = HanedaFlightScraper()

    # 到着便数を30分ごとにカウント
    arrival_counts = scraper.count_arrivals_by_time(flight_data)
    if not arrival_counts:
        print(f"グラフ生成スキップ: {title} - カウントデータなし")
        return None
    ...
    try:
        ...
        print(f"時刻のソート中にエラー: {e}")
        return None # 不正な時刻データがあればスキップ
    ...
    try:
        ...
        print(f"グラフ生成完了: {filepath}")
        return {
            "image_path": f"/static/{filename}",
            "image_data": base64.b64encode(image_bytes).decode('utf-8')
        }
    except Exception as e:
        print(f"グラフ描画または保存中にエラー: {e}")
        plt.close() # エラー時もプロットを閉じる
        return None
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "/flights/plot/t2/json": "T2フライト情報のグラフ(JSON)を取得",
            "/flights/plot/other/json": "T3(その他)フライト情報のグラフ(JSON)を取得",
...
# 結合グラフのエンドポイント (FileResponse) - 必要であれば残す
@app.get("/flights/plot")
async def get_combined_flight_plot():
    """【旧】全フライト情報のグラフ画像を取得するエンドポイント"""
    ...
    plot_result = generate_plot_internal(all_flights, "国際線到着便数（30分間隔）", "combined_flight_graph")
    if not plot_result:
        raise HTTPException(status_code=500, detail="グラフの生成に失敗しました")
    return FileResponse(f"static/{os.path.basename(plot_result['image_path'])}")


@app.get("/flights/plot/t2/json")
async def get_t2_flight_plot_json():
    """T2フライト情報のグラフを取得するエンドポイント（JSON）"""
    global latest_flight_data
    if latest_flight_data["t2_flights"] is None:
        await process_data() # データがなければ全体を更新
    if latest_flight_data["t2_flights"] is None:
        raise HTTPException(status_code=404, detail="T2フライトデータがありません")

    plot_result = generate_plot_internal(latest_flight_data["t2_flights"], "T2 国際線到着便数（30分間隔）", "t2_flight_graph")
    if not plot_result:
        raise HTTPException(status_code=500, detail="T2グラフの生成に失敗しました")
...
@app.get("/flights/plot/other/json")
async def get_other_flight_plot_json():
    """T3(その他)フライト情報のグラフを取得するエンドポイント（JSON）"""
    global latest_flight_data
    if latest_flight_data["other_flights"] is None:
        await process_data() # データがなければ全体を更新
    if latest_flight_data["other_flights"] is None:
        raise HTTPException(status_code=404, detail="T3(その他)フライトデータがありません")

    plot_result = generate_plot_internal(latest_flight_data["other_flights"], "T3(その他) 国際線到着便数（30分間隔）", "other_flight_graph")
    if not plot_result:
        raise HTTPException(status_code=500, detail="T3(その他)グラフの生成に失敗しました")
